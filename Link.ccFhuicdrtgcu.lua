...
local function createSliderControl(parent, item)
	...
	local precision = tonumber(item.precision) or 0
	local pow = 10 ^ precision
	local cur
	if precision > 0 then
		cur = tonumber(item._value) or tonumber(item.default) or (Floor((minVal + maxVal)/2 * pow + 0.5) / pow)
	else
		cur = tonumber(item._value) or tonumber(item.default) or Floor((minVal + maxVal)/2)
	end
	item._value = cur
	local function setPositionFromValue(v, instant)
		...
		if precision > 0 then
			valueLabel.Text = string.format("%." .. precision .. "f", v)
		else
			valueLabel.Text = tostring(Floor(v))
		end
	end
	...
	local function beginDrag(input)
		...
		dragConn = UserInputService.InputChanged:Connect(function(inp)
			...
			if inp.UserInputType == Enum.UserInputType.MouseMovement or inp.UserInputType == Enum.UserInputType.Touch then
				...
				local relX = Clamp((pos.X - absPos.X) / math.max(1, absSize.X), 0, 1)
				local newVal = minVal + relX * (maxVal - minVal)
				if precision > 0 then
					cur = Floor(newVal * pow + 0.5) / pow
				else
					cur = Floor(newVal + 0.5)
				end
				setPositionFromValue(cur, true)
			end
		end)
...
local attackBlacklistOnly = false
...
local function tryResolveReplication()
	local now = tick()
	if now - lastResolveAttempt < 2 then return end
...
spawn(function()
	table.insert(spawnedTasks, coroutine.running())
	while alive do
		local interval = tonumber(hitFlushInterval) or 0.5
	...
	for _, plr in ipairs(players) do
		if plr ~= LocalPlayer then
			...
			if whitelist[uid] then
				...
				if attackBlacklistOnly and not blacklist[uid] then
				else
					local char = plr.Character
					if char then
						local tarRoot = char:FindFirstChild("HumanoidRootPart")
						local tarHum = char:FindFirstChild("Humanoid")
						if tarRoot and tarHum and tarHum.Health > 0 and tarHum:GetState() ~= Enum.HumanoidStateType.Dead then
							...
							if dis <= maxRange then
								if teamCheck and LocalPlayer.Team and plr.Team and LocalPlayer.Team == plr.Team then
									...
									local limb = char:FindFirstChild("Head") or char:FindFirstChild("head") or tarRoot
									local params = RaycastParams.new()
									params.FilterType = Enum.RaycastFilterType.Blacklist
									if character then params.FilterDescendantsInstances = {character} end
									local ok, ray = pcall(function() return workspace:Raycast(rootPart.Position, limb.Position - rootPart.Position, params) end)
									if not ok or not ray or (ray.Instance and ray.Instance:IsDescendantOf(char)) then
										local score = dis
										local head = char:FindFirstChild("Head") or char:FindFirstChild("head")
										if head then score = score - 3.0 end
										local tarHum2 = char:FindFirstChild("Humanoid")
...
local function predictTargetPosition(tarRoot, tarLimb, predictionFactor)
	...
	local lead = Clamp(dist / 20, 0, 1.6) * predictionFactor
...
local function getHeldTool()
	if currentHookedTool and currentHookedTool.Parent and currentHookedTool:IsA("Tool") then
		if weaponInfo[currentHookedTool] and weaponInfo[currentHookedTool].equipped then
			return currentHookedTool
		end
	end
	if LocalPlayer and LocalPlayer.Character then
		...
		for _, obj in ipairs(char:GetChildren()) do
			if obj and obj:IsA("Tool") then
				currentHookedTool = obj
				return obj
			end
		end
	end
	currentHookedTool = nil
	return nil
end
...
local heartbeatConn = RunService.Heartbeat:Connect(function()
	...
	currentCooldown = Clamp(currentCooldown, 0.5, 10)
			...
			if plr and plr.Character and (not whitelist[plr.UserId]) then
				...
				if tarRoot and tarHum and tarHum.Health > 0 and tarHum:GetState() ~= Enum.HumanoidStateType.Dead then
					if (rootPart.Position - tarRoot.Position).Magnitude <= attackRange then
						...
						if character then params.FilterDescendantsInstances = {character} end
						local ok, ray = pcall(function() return workspace:Raycast(rootPart.Position, limb.Position - rootPart.Position, params) end)
						if not ok or not ray or (ray.Instance and ray.Instance:IsDescendantOf(plr.Character)) then
	...
	for idx = 1, maxTargets do
		...
		if idx < maxTargets then task.wait(0.01) end
	end
end)
trackConnection(heartbeatConn)
local categories = {}
local proxZeroEnabled = false
local proxOriginalHold = {}
local proxDescAddedConn = nil
local function applyProximityHoldZero(enable)
	proxZeroEnabled = enable and true or false
	if enable then
		for _, obj in ipairs(workspace:GetDescendants()) do
			if obj:IsA("ProximityPrompt") then
				if proxOriginalHold[obj] == nil then
					proxOriginalHold[obj] = obj.HoldDuration
				end
				pcall(function() obj.HoldDuration = 0 end)
			end
		end
		if not proxDescAddedConn then
			proxDescAddedConn = workspace.DescendantAdded:Connect(function(d)
				if not alive then return end
				if d and d:IsA("ProximityPrompt") then
					if proxOriginalHold[d] == nil then proxOriginalHold[d] = d.HoldDuration end
					pcall(function() d.HoldDuration = 0 end)
				end
			end)
			trackConnection(proxDescAddedConn)
		end
	else
		for prompt, orig in pairs(proxOriginalHold) do
			if prompt and prompt.Parent then
				pcall(function() prompt.HoldDuration = orig end)
			end
		end
		proxOriginalHold = {}
		if proxDescAddedConn then
			pcall(function() proxDescAddedConn:Disconnect() end)
			proxDescAddedConn = nil
		end
	end
end
local camShakeDisableEnabled = false
local camShakeLoopRunning = false
local function applyCamShakeDisable(enable)
	camShakeDisableEnabled = enable and true or false
	if camShakeDisableEnabled then
		if camShakeLoopRunning then return end
		camShakeLoopRunning = true
		task.spawn(function()
			while alive and camShakeDisableEnabled do
				local ok, camShakeScript = pcall(function()
					return ReplicatedStorage:FindFirstChild("Features") and ReplicatedStorage.Features:FindFirstChild("CameraShake") and ReplicatedStorage.Features.CameraShake:FindFirstChild("CameraShakeClient")
				end)
				if ok and camShakeScript then
					pcall(function() camShakeScript:Destroy() end)
				end
				for i = 1, 20 do
					if not (alive and camShakeDisableEnabled) then break end
					task.wait(0.1)
				end
			end
			camShakeLoopRunning = false
		end)
	end
end
...
local function openBlacklistPopup()
	if not ScreenGui or not ScreenGui.Parent then return end
	local POP_W, POP_H = 420, 300
	local popup = Instance.new("Frame", ScreenGui)
	popup.AnchorPoint = Vector2.new(0.5, 0.5)
	...
	local function makePlayerButton(plr)
		...
		status.Font = Enum.Font.Gotham
		status.TextSize = 13
		status.Text = blacklist[plr.UserId] and "已加入黑名单" or "未加入"
		status.TextColor3 = Color3.fromRGB(200,200,200)
		status.TextXAlignment = Enum.TextXAlignment.Left
		status.ZIndex = details.ZIndex + 1
		local open = false
		...
		trackConnection(btn.MouseButton1Click:Connect(function()
			if blacklist[plr.UserId] then
				blacklist[plr.UserId] = nil
			else
				blacklist[plr.UserId] = true
			end
			updateVisual()
			toggleDetails()
		end))
		blacklistButtons[plr.UserId] = btn
		updateVisual()
	end
	for _, plr in ipairs(Players:GetPlayers()) do
		if plr ~= LocalPlayer then
	...
	trackConnection(Players.PlayerRemoving:Connect(function(plr)
		local b = blacklistButtons[plr.UserId]
		if b then pcall(function() b:Destroy() end) end
		blacklist[plr.UserId] = nil
		blacklistButtons[plr.UserId] = nil
	end))
	local closeBtn = Instance.new("TextButton", popup)
	closeBtn.Size = UDim2.new(0, 100, 0, 30)
	closeBtn.Position = UDim2.new(1, -116, 1, -40)
...
categories = {
	{
		...
		items = {
			...
			},
			{
				...
				desc = "攻击冷却",
				onChange = function(v)
					attackCoolDown = math.max(0.5, tonumber(v) or attackCoolDown)
				end
			},
			{
				title = "Only Attack Blacklist",
				type = "toggle",
				desc = "仅攻击黑名单中的玩家",
				defaultOn = attackBlacklistOnly,
				onToggle = function(on)
					attackBlacklistOnly = (on == true)
				end
			},
			{
			...
			},
			{
				...
				desc = "发送击中包延迟，循环发送间隔",
				onChange = function(v)
					hitFlushInterval = math.max(0.02, tonumber(v) or hitFlushInterval)
				end
			},
			{
		...
		items = {
			{
				...
				type = "toggle",
				desc = "E键互动延迟修改",
				defaultOn = proxZeroEnabled,
				onToggle = function(on)
					applyProximityHoldZero(on == true)
				end
			},
			{
				title = "禁用相机抖动",
